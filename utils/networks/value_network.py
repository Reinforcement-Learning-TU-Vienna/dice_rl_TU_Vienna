...
from tf_agents import specs # change
...
class ValueNetwork(network.Network):
  ...
  def call(self, inputs, step_type=(), network_state=(), training=False):
    ...
    for single_input, input_spec in zip(flat_inputs, self._flat_specs):
      is_categorical_spec = (
        specs.tensor_spec.is_discrete(input_spec) and
        specs.tensor_spec.is_bounded(input_spec) and
        input_spec.shape == [] and
        input_spec.minimum == 0
      ) # change
      # if common_lib.is_categorical_spec(input_spec):
      if is_categorical_spec: # change
