...
class TabularCoinDice(object):
  ...
  def prepare_dataset(self, dataset: dataset_lib.OffpolicyDataset,
    ...
    initial_target_probs = target_policy.distribution(
        tfagents_initial_env_step).action.probs_parameter() # type: ignore
    initial_target_probs = tf.cast(initial_target_probs, dtype=tf.float32)
    next_target_probs = target_policy.distribution(
        tfagents_next_env_step).action.probs_parameter() # type: ignore
    next_target_probs = tf.cast(next_target_probs, dtype=tf.float32)
