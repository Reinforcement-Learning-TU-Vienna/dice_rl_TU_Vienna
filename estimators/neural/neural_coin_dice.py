...
class NeuralCoinDice(NeuralDualDice):
  ...
  def __init__(self,
    ...
    super().__init__(dataset_spec, nu_network, zeta_network, nu_optimizer,
  ...
  def train_step(self, initial_env_step: dataset_lib.EnvStep,
    ...
    with tf.GradientTape(
      ...
      tape.watch([variable.value for variable in self._nu_network.variables])
      tape.watch([variable.value for variable in self._zeta_network.variables])
      tape.watch([variable.value for variable in self._weight_network.variables])
      ...
      left = self._alpha - 100 * tf.ones_like(self._two_sided_limit)
      right = self._alpha + 100 * tf.ones_like(self._two_sided_limit)
      for _ in range(32):
        mid = 0.5 * (left + right)
