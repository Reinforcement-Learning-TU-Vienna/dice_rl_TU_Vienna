...
def main(argv):
  ...
  create_dataset(env_name, seed, num_trajectory, max_trajectory_length, alpha, tabular_obs, save_dir, load_dir, force)

def create_dataset(env_name, seed, num_trajectory, max_trajectory_length, alpha, tabular_obs, save_dir, load_dir, force):
  ...
  return create_dataset_loaded(
    seed, num_trajectory, max_trajectory_length, save_dir, force,
    hparam_str, dataset)

def create_dataset_loaded(
    seed, num_trajectory, max_trajectory_length, save_dir, force,
    hparam_str, dataset):

  # I commented this out above
  directory = os.path.join(save_dir, hparam_str)
  if tf.io.gfile.isdir(directory) and not force:
    raise ValueError('Directory %s already exists. Use --force to overwrite.' %
                     directory)
  ...
  for batch_num in range(1 + (num_trajectory - 1) // batch_size):
    ...
    print('num episodes collected:',
      '\n', write_dataset.num_total_episodes)
    print('num steps collected:',
      '\n', write_dataset.num_steps)

    estimate = estimator_lib.get_fullbatch_average(write_dataset)
    print('per step avg on offpolicy data',
      '\n', estimate)
    estimate = estimator_lib.get_fullbatch_average(write_dataset,
                                                   by_steps=False)
    print('per episode avg on offpolicy data',
      '\n', estimate)
    print()
  ...
  print('num loaded steps',
      '\n', new_dataset.num_steps)
  print('num loaded total steps',
      '\n', new_dataset.num_total_steps)
  print('num loaded episodes',
      '\n', new_dataset.num_episodes)
  print('num loaded total episodes',
      '\n', new_dataset.num_total_episodes)

  estimate = estimator_lib.get_fullbatch_average(new_dataset)
  print('per step avg on saved and loaded offpolicy data',
      '\n', estimate)
  estimate = estimator_lib.get_fullbatch_average(new_dataset,
                                                 by_steps=False)
  print('per episode avg on saved and loaded offpolicy data',
      '\n', estimate)
  print()
